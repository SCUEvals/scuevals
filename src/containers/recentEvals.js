...
class RecentEvals extends Component {
  ...
  render() {
    ...
    if (evals && quartersList && coursesList && coursesList.departmentsListLoaded && professorsList) return (
      ...
      <ul style={{minWidth: '478px'}} className="list-group">
        {evals.map(evaluation => {
          ...
          let avgStyle={strokeDashoffset: this.calculatePath(average)};
            return (
              <li key={evaluation.id} class="list-group-item d-flex justify-content-between align-items-center">
                <div className='flex col-2'>
                  {quartersList.object[evaluation.quarter_id].label }
                </div>
                <div className='flex col-4'>
                  <Link to={`/courses/${evaluation.course.id}`} >{coursesList.object[evaluation.course.id].label}</Link>
                </div>
                <div className='flex col-4'>
      ...
      <h5>Loading Most Recent Evaluations...</h5>
    );
  }
}
